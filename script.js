class CGMOrderManager {
  ...
  applyDeliveryCycleLabel() {
    ...
    const configured =
      typeof CONFIG !== "undefined" && CONFIG.DELIVERY_CYCLE ? String(CONFIG.DELIVERY_CYCLE).trim() : ""
    ...
    if (ordersCloseEl) {
      const closeLabel =
        typeof CONFIG !== "undefined" && CONFIG.ORDER_CLOSES_DATE && String(CONFIG.ORDER_CLOSES_DATE).trim()
          ? String(CONFIG.ORDER_CLOSES_DATE).trim()
          : ""
  ...
  updateOrderFormState() {
    ...
    if (!this.ordersEnabled) {
      ...
      if (!overlay) {
        ...
        overlay.innerHTML =
          '<div class="disabled-message"><i class="fas fa-pause-circle"></i><br>Currently not accepting orders</div>'
  ...
  async verifyPaymentWithBackend(file, expectedAmount, transactionId) {
      ...
      reader.onload = async () => {
        try {
          ...
          const verificationData = {
            ...
            expectedRecipient: CONFIG.UPI_RECIPIENT_NAME,
            expectedUpiId: CONFIG.UPI_ID,
          ...
          // For production, we can't read response due to no-cors, so we'll skip verification
          if (isProd) {
            console.log('Production mode: verification will be handled server-side')
            resolve({ 
              message: 'Verification handled server-side',
              serverSide: true 
            })
            return
          }

          // For development/local testing, we can get the response
          ...
          if (!response.ok) {
            throw new Error(`Backend verification error: ${response.status}`)
          }

          const result = await response.json()
          
          if (result.verification) {
            console.log('Payment verification result:', result.verification)
            resolve(result.verification)
          } else {
            resolve({ 
              message: 'Verification processed by backend',
              serverSide: true 
            })
          }
        } catch (error) {
  ...
  async handleFormSubmit(e) {
    ...
    // Verify payment screenshot with backend (Gemini AI on server-side)
    if (paymentScreenshot && paymentScreenshot.files.length > 0) {
      try {
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Verifying payment...'
        const verification = await this.verifyPaymentWithBackend(
        ...
        // Handle server-side verification (production mode)
        if (verification.serverSide) {
          // Verification will be handled by backend and stored directly in sheets
          orderData.verificationStatus = 'PENDING_SERVER'
          orderData.verificationIssues = 'Verification handled by server'
          console.log('Verification will be processed server-side')
        } else {
          // Store verification result in order data (development mode)
          orderData.paymentVerification = verification
          orderData.verificationStatus = verification.allChecksPass ? 'VERIFIED' : 'FAILED'
          
          // Build a summary of verification for Google Sheets
          if (verification.allChecksPass === false) {
            const issues = []
            if (!verification.paymentSuccess) issues.push("Payment not successful")
            if (!verification.amountMatches) issues.push(`Amount mismatch (Expected: ₹${orderData.totalAmount}, Found: ₹${verification.amount})`)
            if (!verification.recipientMatches) issues.push(`Recipient mismatch (Expected: ${CONFIG.UPI_RECIPIENT_NAME}, Found: ${verification.recipient})`)
            if (!verification.transactionIdMatches) issues.push(`Transaction ID mismatch (Expected: ${transactionId}, Found: ${verification.transactionId})`)
            if (!verification.isWithin10Minutes) issues.push("Payment not within last 10 minutes")
            
            orderData.verificationIssues = issues.join('; ')
            console.warn('Payment verification issues:', issues)
          } else {
            orderData.verificationIssues = 'None'
          }
        }
        ...
        // Store verification error in order data
        orderData.paymentVerification = {
          error: error.message,
          verified: false
        }
        orderData.verificationStatus = 'ERROR'
        orderData.verificationIssues = `Verification error: ${error.message}`
      }
    } else {
      // No screenshot provided (shouldn't happen due to validation, but just in case)
      orderData.verificationStatus = 'NOT_VERIFIED'
      orderData.verificationIssues = 'No screenshot provided'
    }
  ...
  showSuccessModal(orderData) {
    ...
    const totalSavings = perSensorSavings * (Number.parseInt(orderData.quantity) || 0)

    // Determine verification status display
    let verificationBadge = ''
    if (orderData.verificationStatus === 'VERIFIED') {
      verificationBadge = '<strong style="color: #48bb78;">✓ Payment Verified</strong><br>'
    } else if (orderData.verificationStatus === 'FAILED') {
      verificationBadge = '<strong style="color: #f59e0b;">⚠ Payment verification needs review</strong><br>'
    } else if (orderData.verificationStatus === 'ERROR') {
      verificationBadge = '<strong style="color: #6b7280;">ℹ Payment will be reviewed manually</strong><br>'
    } else if (orderData.verificationStatus === 'PENDING_SERVER') {
      verificationBadge = '<strong style="color: #6b7280;">ℹ Payment verification in progress</strong><br>'
    }
  ...
  updateSummary() {
    ...
    const totalSensors = this.orders.reduce((sum, order) => sum + Number.parseInt(order.quantity), 0)
    ...
    if (this.orders.length > 0) {
      ...
      lastOrderTimeElement.classList.add("time-stat")
    } else {
      lastOrderTimeElement.textContent = "-"
      lastOrderTimeElement.classList.remove("time-stat")
    }
  }
  ...
  calculateTotalSavings() {
    return this.orders.reduce((total, order) => {
      ...
      if (sensorInfo) {
        return total + sensorInfo.savings * Number.parseInt(order.quantity)
      }
      return total
    }, 0)
  }
  ...
  renderServerOrders(serverOrders) {
    ...
    const html = serverOrders
      .slice(0, CONFIG.ORDERS_FETCH_LIMIT || 10)
      .map((order) => {
        const sensorType = this.getSensorTypeName(order.sensorType)
